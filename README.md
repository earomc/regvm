...
There is only one type of value, that is an unsigned integer.
...
Plain text is considered a `marker` instruction with the text as its name. You can jump to markers with the `go` instruction